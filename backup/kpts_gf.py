...
def greens_b_vector_ea_rhf(cc,p,kp=None):
    ...
    vector2 = np.zeros((nkpts,nkpts,nocc,nocc,nvir,nvir),dtype=ds_type)
    if p < nocc:
...
def greens_e_vector_ea_rhf(cc,p):
    ...
    if p < nocc:
        ...
        for ki in range(nkpts):
            for kj in range(nkpts):
                vector2[ki, kj] += 2*cc.l2[ki,kj,kp,p,:,:,:] - \
                                     cc.l2[kj,ki,kp,:,p,:,:]
        pass

    else:
        vector1[ p-nocc ] = -1.0
        vector1 += np.einsum('ia,i->a', cc.l1[kp], cc.t1[kp,:,p-nocc])
        for kl in range(nkpts):
            for kc in range(nkpts):
                vector1 += 2 * np.einsum('klca,klc->a', cc.l2[kp,kl,kc], \
                           cc.t2[kp,kl,kc,p,:,:,:,p-nocc])
                vector1 -= np.einsum('klca,lkc->a', cc.l2[kp,kl,kc], \
                           cc.t2[kp,kl,kc,:,:,:,p-nocc])

        for kj in range(nkpts):
            vector2[kp,kj,:,p-nocc,:] += -2.*cc.l1[kj]

        for ki in range(nkpts):
            vector2[ki,kp,:,:,p-nocc] += cc.l1[ki]
            
            for kj in range(nkpts):
                vector2[ki,kj] += 2*np.einsum('k,jkba->jab', \
                                  cc.t1[kp,:,p-nocc], cc.l2[ki,kj,kp,:,:,:,:])
...
def greens_b_vector_ip_rhf(cc,p,kp=None):
    ...
    vector2 = np.zeros((nkpts,nkpts,nocc,nocc,nvir,nvir),dtype=complex)
...
def greens_e_vector_ip_rhf(cc,p,kp=None):
    ...
    if p < nocc:
        vector1[p] = -1.0
        vector1 += np.einsum('ia,a->i', cc.l1[kp], cc.t1[kp,p,:])
        for kl in range(nkpts):
            for kc in range(nkpts):
                vector1 += 2 * np.einsum('ilcd,lcd->i', \
                       cc.l2[kp,kl,kc], cc.t2[kp,kl,kc,p,:,:,:])
                vector1 -= np.einsum('ilcd,ldc->i',   \
                       cc.l2[kp,kl,kc], cc.t2[kp,kl,kc,p,:,:,:])
      
        for kj in range(nkpts):
            vector2[kp,kj,p,:,:] += -2*cc.l1[kj]

        for ki in range(nkpts):
            # kj == kk == kp, ki == kb
            vector2[ki,kp,:,p,:] +=  cc.l1[ki]

            for kj in range(nkpts):
                # kc == kk == kp
                vector2[ki,kj] += 2*np.einsum('c,ijcb->ijb', \
                       cc.t1[kp,p,:], cc.l2[ki,kj,kp,:,:,:,:])
                vector2[ki,kj] -= np.einsum('c,jicb->ijb', \
                       cc.t1[kp,p,:], cc.l2[ki,kj,kp,:,:,:,:])
    else:
        vector1 += -cc.l1[kp,:,p-nocc]
        for ki in range(nkpts):
            for kj in range(nkpts):
                vector2[ki, kj] += -2*cc.l2[ki,kj,kp,:,:,p-nocc,:] + \
                                      cc.l2[kj,ki,kp,:,:,p-nocc,:]
    return cc.amplitudes_to_vector_ip(vector1,vector2)
...
def initial_ip_guess(cc):
    ...
    #Is there nkpts x3? the function a_to_v_ip would suggest x2
    vector2=np.zeros((nkpts,nkpts,nocc,nocc,nvir,nvir),dtype=complex)
    return cc.amplitudes_to_vector_ip(vector1,vector2)

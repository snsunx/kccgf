    ...
    if p < nocc:
        ...
        for kk in range(nkpts):
            for kl in range(nkpts):
                ...
                ka = kconserv[kp,kl,kk]
                
                vector1 += 2 * np.einsum('klca,klc->a', l2[kk,kl,kp], \
                           cc.t2[kk,kl,kp,:,:,:,p-nocc])
                vector1 -= np.einsum('klca,lkc->a', l2[kk,kl,kp], \
                           cc.t2[kl,kk,kp,:,:,:,p-nocc])
        ...
        for kj in range(nkpts):
            for kb in range(nkpts):
                ...
                vector2[kj,kb] += 2*np.einsum('k,jkba->jab', \
                                  cc.t1[kp,:,p-nocc], l2[kj,kp,kb,:,:,:,:])
                vector2[kj,kb] -= np.einsum('k,jkab->jab', \

...
def greens_e_vector_ea_rhf(cc,p):
    ...
    if p < nocc:
        ...
        #print '#################vector2[:,p-nocc,:] += -2.*l1####################'
        #print vector2       

        vector2[:,:,p-nocc] += l1

        #print '#################vector2[:,:,p-nocc] += l1####################'
        #print vector2         

        tempvec2 = np.zeros((nocc,nvir,nvir),dtype=ds_type)
        vector2 += 2*np.einsum('k,jkba->jab', cc.t1[:,p-nocc], l2)
        tempvec2 += 2*np.einsum('k,jkba->jab', cc.t1[:,p-nocc], l2)
        
        print '###############2*np.einsum(k,jkba->jab, cc.t1[:,p-nocc], l2)####################'
        print tempvec2

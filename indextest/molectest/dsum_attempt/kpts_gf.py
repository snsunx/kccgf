...
def greens_e_vector_ip_rhf(cc,p,kp=None):
    nkpts, nocc, nvir = cc.t1.shape
    vector1 = np.zeros((nkpts,nocc),dtype='complex128')
    ...
    if p < nocc:
        for ki in range(nkpts):
            vector1[ki] = -1.0
            vector1[ki] += np.einsum('ia,a->i', l1[kp], cc.t1[kp,p,:])
            for kl in range(nkpts):
                for kc in range(nkpts):
                    kconserv = cc.khelper.kconserv
                    kd = kconserv[ki,kl,kc]
                    vector1[ki] += 2 * np.einsum('ilcd,lcd->i', \
                               l2[ki,kl,kc], cc.t2[ki,kl,kc,p,:,:,:])
                               #l2[kp,kl,kc], cc.t2[kl,kc,kd,p,:,:,:])
                    vector1[ki] -= np.einsum('ilcd,ldc->i',   \
                               l2[ki,kl,kc], cc.t2[ki,kl,kd,p,:,:,:]) 
                               #l2[kp,kl,kc], cc.t2[kl,kd,kc,p,:,:,:])
            for kj in range(nkpts):
                vector2[ki,kj,p,:,:] += -2*l1[kj]
        for ki in range(nkpts):
            # kj == kk == kp, ki == kb

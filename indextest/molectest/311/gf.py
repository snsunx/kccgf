...
def greens_b_vector_ip_rhf(cc,p):
    ...
    vector2 = np.zeros((nocc,nocc,nvir),dtype='complex128')
    if p < nocc:
        vector1[ p ] = 1.0
    else:
...
def greens_e_vector_ip_rhf(cc,p):
    ...
    #print '#################################'
    #print 'max t1 molec', np.max(np.abs(cc.t1))
    ...
    def solve_ip(self, ps, qs, omegas):
        ...
        for ip, p in enumerate(ps):
            ...
            for iw, omega in enumerate(omegas):
                ...
                Sw, info = spla.gmres(Ax, b_vector, x0=Sw, tol=10e-17, M=mx)
                for iq,q in enumerate(qs):
                    gfvals[ip,iq,iw]  = -np.dot(e_vector[iq],Sw)
        if len(ps) == 1 and len(qs) == 1:
            print 'ip gfs',gfvals[0,0,:]
            return gfvals[0,0,:]
        else:
            print 'ip gfs',gfvals
            return gfvals
